...
def extract_elastic_vit(args):
    ...
    for net in args.elastic_nets:
        ...
        elastic_ckpt_name = Path(args.intact_ckpt).stem + f'_{net}_with_head.pth'
...
def extract_elastic_swin(args, remove_dynamic=False):
    ...
    for net in args.elastic_nets:
        ...
        for k, v in intact_model.items():
            ...
            # patch_embed
            if 'patch_embed' in k:
                k_ = k.replace('norm.bias', 'bias').replace('norm.weight',
                                                            'weight') if remove_dynamic else k
                v_ = v[:width_init]
                elastic_state[k] = v_
                print(k, '->', k_, v.shape, '->', v_.shape)
            # blocks of stages
