...
def main():
    ...
    parser.add_argument('--local_rank', default=0, type=int, help='node rank for distributed parallel')
    ...
    parser.add_argument("--topk", type=int, default=64, help="topk")
    ...
    network = utils.MultiCropWrapper(network, MDINOHead(
        1024, out_dims = [8192, 16384, 32768, 65536],
    ))
    network.head.set_index(network.backbone.embed_dim, 2048)  
    network.cuda(args.local_rank)
    utils.load_pretrained_weights(network, pretrained_path, checkpoint_key='teacher')   
    ...
    data_file = os.path.join(args.save_path, f'memory_{args.type}.pth')
    if os.path.exists(data_file):
        ...
        for data in tqdm(train_dataloader):
            ...
            feat_idx = args.out_dim // 8192 - 1
            feature =  network(img)[feat_idx]
            feature = feature[:, :1].contiguous() if args.type == 'patch' else feature[:, 0].contiguous()
