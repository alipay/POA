...
from resnet import resnet_dynamic
...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='vit_small', type=str, choices=['vit_tiny', 'vit_small', 'vit_base', 
        'vit_large', 'swin_tiny','swin_small', 'swin_base', 'swin_large', 'resnet'], help='Architecture.')
...
def build_dataset(is_train, args):
    ...
    percent_name = Path(args.target_list_path).stem
    if is_train:
        ...
        target_list = [] 
    dataset = ImageFolder(root, transform=transform, class_num=args.class_num, target_list=target_list, percent_name=percent_name)
    nb_classes = 1000
...
def main(args, logger):
    utils_for_semi.init_distributed_mode(args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if dist.get_rank() == 0: 
            logger.info(f'Max accuracy [{epoch + 1}]: {max_accuracy:.2f}%')
