...
from poa.eval.unsupervised_cls.vision_transformer import vit_small, vit_base, vit_large
from poa.eval.unsupervised_cls.resnet import resnet_dynamic
...
def main_eval(args):
    ...
    if args.arch == 'vit_small':
        ...
        model = vit_base(patch_size=args.patch_size, num_classes=0)
    elif args.arch == 'vit_large':
        model = vit_large(patch_size=args.patch_size, num_classes=0)
    elif args.arch == 'resnet50':
        model = resnet_dynamic(
            min_depths=[3, 4, 6, 3],
            max_depths=[3, 8, 36, 3],
            stem_channels=64,
            base_channels=64,
        ...
        model.get_by_type('R50')
        model.embed_dim = 2048
        checkpoint_key = 'teacher'
    else:
        raise Exception('Unsupported arch: {}'.format(args.arch)) 
    embed_dim = model.embed_dim
    print(f"Model {args.arch} {args.patch_size}x{args.patch_size} built.")
    #model = utils.MultiCropWrapper(model, MDINOHead(
    #    max_in_dim=1024,
    #    out_dims=[args.out_dim, ], 
    #    ))
    model = utils.MultiCropWrapper(model, None)
    model.cuda(args.local_rank)
    #model.head.set_index(embed_dim, 2048)
    
    utils.load_pretrained_weights(model, args.pretrained_weights, checkpoint_key)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='vit_small', type=str, choices=['vit_small', 'vit_base', 'vit_large', 'resnet50'], help='Architecture.')
