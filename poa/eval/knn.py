...
from poa.data import SamplerType, make_data_loader, make_dataset
from poa.data.transforms import make_classification_eval_transform
from poa.eval.metrics import AccuracyAveraging, build_topk_accuracy_metric
from poa.eval.setup import get_args_parser as get_setup_args_parser
from poa.eval.setup import setup_and_build_model
from poa.eval.utils import ModelWithNormalize, evaluate, extract_features
...
def eval_knn(
    ...
    if len(list(net_types)) > 0:
        net_ids = net_types
    else:
        net_ids = model.model.get_eval_arch()

    logger.info("Extracting features for train set...")
    train_features_list, train_labels_list = extract_features(
        model, net_ids, train_dataset, batch_size, num_workers, gather_on_cpu=gather_on_cpu
    )
    ...
    num_classes = train_labels_list[0].max() + 1
    ...
    results_dict_list = []
    for i, (train_features, train_labels) in enumerate(zip(train_features_list, train_labels_list)):
        ...
        #model.model.get_index(net_ids[i])
        model.model.get_by_type(net_ids[i])
...
def eval_knn_with_model(
    ...
    with torch.cuda.amp.autocast(dtype=autocast_dtype):
        results_dict_knn_list = eval_knn(
            model=model,
            net_types=net_types,
    ...
    results_dict = {}
    for results_dict_knn in results_dict_knn_list:
...
if __name__ == "__main__":
    description = "POA k-NN evaluation"
