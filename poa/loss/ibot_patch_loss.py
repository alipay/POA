...
class iBOTPatchLoss(nn.Module):
    def __init__(self, patch_out_dims, student_temp=0.1, center_momentum=0.9):
        super().__init__()
        self.student_temp = student_temp
        self.center_momentum = center_momentum
        if isinstance(patch_out_dims, int):
            patch_out_dims = [patch_out_dims]
        self.num_centers = len(patch_out_dims)
        for i in range(self.num_centers):
            self.register_buffer(f"center_{i}", torch.zeros(1, 1, patch_out_dims[i]))
        self.updated = True
    ...
    def softmax_center_teacher(self, teacher_patch_tokens, teacher_temp):
        ...
        if isinstance(teacher_patch_tokens, torch.Tensor):
            teacher_patch_tokens = [teacher_patch_tokens]
        assert len(teacher_patch_tokens) == self.num_centers
        outputs = []
        for i, teacher_patch_token in enumerate(teacher_patch_tokens):
            center = getattr(self, f"center_{i}")
            outputs.append(F.softmax((teacher_patch_token - center) / teacher_temp, dim=-1))
    ...
    def reduce_center_update(self, teacher_patch_tokens):
        ...
        if isinstance(teacher_patch_tokens, torch.Tensor):
            teacher_patch_tokens = [teacher_patch_tokens]
        self.len_teacher_patch_tokens = len(teacher_patch_tokens[0])
        for i, teacher_patch_token in enumerate(teacher_patch_tokens):
            self.async_batch_center[i] = torch.sum(teacher_patch_token.mean(1), dim=0, keepdim=True)
            if dist.is_initialized():
    ...
    def apply_center_update(self):
        if self.updated is False:
            ...
            for i in range(self.num_centers):
                ...
                _t = self.async_batch_center[i] / (self.len_teacher_patch_tokens * world_size)
                center = getattr(self, f"center_{i}")
